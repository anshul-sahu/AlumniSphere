...
public class InternshipService {
	...
	public List<StudentInternshipStatusDto> collectStudentStatus(Integer userId){
		List<InternshipApplication> list = internAppRepo.findAllApplicationsByUserId(userId);
		List<StudentInternshipStatusDto> result = new ArrayList<>();
		for(InternshipApplication next : list) {
			StudentInternshipStatusDto dto = new StudentInternshipStatusDto();
			dto.setAppStatus(next.getStatus());
			Internship internship = internRepo.findById(next.getInternship().getInternshipId()).get();
